...
app.get('/ships', async (req, res) => {
  try {
    const data = await client.query('SELECT * from ships');
...
app.post('/ships', async (req, res) => {
  try {
    ...
    const data = await client.query(`
    INSERT INTO ships ('name, weapons, docked, size, class, image ')
    VALUES($1, $2, $3, $4, $5, $6)
    RETURNING *
      `,
      [newName, newWeapons, newDocked, newSize, newClass, newImage]);
    res.json.apply(data.rows[0]);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
